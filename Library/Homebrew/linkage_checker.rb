...
class LinkageChecker
  ...
  def check_formula_deps
    ...
    unnecessary_deps = declared_deps_full_names.reject do |full_name|
      ...
      name = full_name.split("/").last
      # Optimize string matching by avoiding intermediate array allocations (keys, map, split)
      # and leveraging short-circuit evaluation via `.any?`.
      @brewed_dylibs.each_key.any? { |l| l.end_with?("/#{name}") || l == name }
    end
